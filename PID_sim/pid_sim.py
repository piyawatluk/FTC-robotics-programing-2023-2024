...
while i < t:
    ...
    if not target_approached and mm <= current_value < mp:
        ...
        logger.info("target approach time %s angle approach %s",
                    i, current_value)
        target_approached = True
    if current_value > mp:
        print("angle overshoot above maximum accept angle for",
              current_value, "at time", i)
        logger.critical(
            "Angle overshoot above maximum accepted angle for %s at time %s", current_value, i)
...
plt.plot(new_time_points, smoothed_por_value,
         label='Proportional Term (Smoothed)')
plt.plot(new_time_points, smoothed_intregal_value,
         label='Integral Term (Smoothed)')
